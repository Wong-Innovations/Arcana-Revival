...
public class ItemMagicDust extends ItemBase {
    ...
    private void doSparkles(Player player, Level world, BlockPos pos, double hitX, double hitY, double hitZ, InteractionHand hand, IDustTrigger trigger, IDustTrigger.Placement place) {
        ...
        world.playLocalSound(pos.getX(), pos.getY(), pos.getZ(), ModSounds.dust.get(), SoundSource.PLAYERS, 0.33f, 1.0f + (float) world.random.nextGaussian() * 0.05f, false);
        List<BlockPos> sparkles = List.of(pos);
        if (sparkles != null) {
            Vec3 v3 = new Vec3(pos.getX(), pos.getY(), pos.getZ()).add(hitX, hitY, hitZ);
