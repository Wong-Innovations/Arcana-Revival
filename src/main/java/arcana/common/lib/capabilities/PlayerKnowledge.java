package arcana.common.lib.capabilities;
...
public class PlayerKnowledge {
    private static class DefaultImpl implements IPlayerKnowledge {
        ...
        @Override
        public boolean setResearchStage(@NotNull String res, int stage) {
            if (!this.research.contains(res) || stage <= 0) {
                return false;
            }
        ...
        @Override
        public boolean addKnowledge(@NotNull EnumKnowledgeType type, ResearchCategory category, int amount) {
        ...
        @Override
        public int getKnowledge(@NotNull EnumKnowledgeType type, ResearchCategory category) {
            String key = this.getKey(type, category);
            int c = this.knowledge.getOrDefault(key, 0);
            return (int) Math.floor(c / (double) type.getProgression());
        }

        @Override
        public int getKnowledgeRaw(@NotNull EnumKnowledgeType type, ResearchCategory category) {
            String key = this.getKey(type, category);
            return this.knowledge.getOrDefault(key, 0);
        }
    ...
    public static class Provider implements ICapabilitySerializable<CompoundTag> {
        public static final ResourceLocation NAME = new ResourceLocation(Arcana.MODID, "knowledge");
        ...
        public @Nonnull <T> LazyOptional<T> getCapability(@Nonnull Capability<T> capability, @Nullable Direction facing) {
            if (capability == ModCapabilities.KNOWLEDGE) {
