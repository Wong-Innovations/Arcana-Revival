...
import com.wonginnovations.arcana.client.gui.FociForgeScreen;
import com.wonginnovations.arcana.network.Connection;
import com.wonginnovations.arcana.network.PkFociForgeAction;
import com.wonginnovations.arcana.systems.spell.modules.SpellModule;
import com.wonginnovations.arcana.systems.spell.modules.core.Connector;
...
public class SpellState {
    ...
    public static final ResourceLocation SPELL_RESOURCES = new ResourceLocation(Arcana.MODID, "textures/gui/container/foci_forge_minigame.png");
    ...
    public void render(GuiGraphics guiGraphics, int guiLeft, int guiTop, int spellLeft, int spellTop, int width, int height, int mouseX, int mouseY) {
        ...
        guiGraphics.pose().translate(spellLeft, spellTop, 0);
        // draw background
        ...
        for (float bg_x = start_x; bg_x < width + 16; bg_x += 16) {
            for (float bg_y = start_y; bg_y < height + 16; bg_y += 16) {
                guiGraphics.blit(SPELL_RESOURCES, (int)Math.floor(bg_x), (int)Math.floor(bg_y), bg_texX, 0, 16, 16);
            }
        }
        ...
        if (currentSpell != null) {
            ...
            while(!connectQueue.isEmpty()) {
                ...
                guiGraphics.pose().translate(rootPoint.x, rootPoint.y, 0);
                // sprite rendered from (0,0) to (x, 0), so we rotate the matrix to draw accordingly
                float angle = (float) Math.atan2(basePoint.y - rootPoint.y, basePoint.x - rootPoint.x);
                guiGraphics.pose().mulPose(new Quaternionf(0, 0, 1, angle));
                guiGraphics.blit(SPELL_RESOURCES, -8, -8, 176, 16, 16, 16);
                guiGraphics.blit(SPELL_RESOURCES, (int)distance - 8, -8, 208, 16, 16, 16);
                if (distance > 16) {
                    guiGraphics.pose().translate(8,0,0);
                    guiGraphics.pose().scale(((float)distance - 16.0f) / 16.0f,1,  1);
                    guiGraphics.blit(SPELL_RESOURCES, 0, -8, 192, 16, 16, 16);
                }
            ...
            while(!renderQueue.isEmpty()) {
                ...
                if (!floating.containsKey(next)) {
                    next.renderInMinigame(guiGraphics, mouseX, mouseY, false);
                } else { // current player is floating module
                    RenderSystem.setShaderColor(1.0f, 1.0f, 1.0f, .75f);
                    next.renderInMinigame(guiGraphics, mouseX, mouseY, true);
        ...
        if (activeModule instanceof Connector) {
            activeModule.renderInMinigame(guiGraphics, mouseX, mouseY, (!activeModule.unplaced));
        }
        ...
        if (activeModule != null) {
            activeModule.renderUnderMouse(guiGraphics, mouseX, mouseY, (!activeModule.unplaced));
        }

        if (currentSpell != null) {
            for (int i = 0; i < 9; i++) {
                guiGraphics.blit(FociForgeScreen.BG,
