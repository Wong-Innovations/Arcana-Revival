...
public interface AspectHandler extends INBTSerializable<CompoundTag> {
	...
	default AspectHolder findFirstHolderContaining(Aspect aspect) {
		return findFirstHolderMatching(holder -> holder.getStack().getAspect() == aspect || (holder.getWhitelist() != null && holder.getWhitelist().contains(aspect)));
	}
	...
	*/
	...
	static LazyOptional<AspectHandler> getOptional(@Nullable ICapabilityProvider holder) {
		...
		return holder.getCapability(AspectHandlerCapability.ASPECT_HANDLER_CAPABILITY, null);
	}
	...
	static AspectHandler getFrom(@Nullable ICapabilityProvider holder) {
		if (holder == null)
			return null;
		return holder.getCapability(AspectHandlerCapability.ASPECT_HANDLER_CAPABILITY, null).orElse(null);
	}
}