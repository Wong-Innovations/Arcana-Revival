...
public class InventoryUtils {
    public static boolean consumePlayerItem(Player player, ItemStack item, boolean nocheck, boolean ore) {
        ...
        for (int var2 = 0; var2 < player.getInventory().getContainerSize(); ++var2) {
            if (checkEnchantedPlaceholder(item, player.getInventory().getItem(var2)) || areItemStacksEqual(player.getInventory().getItem(var2), item, new ArcanaInvHelper.InvFilter(false, !item.hasTag(), ore, false).setRelaxedNBT())) {
    ...
    public static boolean isPlayerCarryingAmount(Player player, ItemStack stack, boolean ore) {
        ...
        for (int var2 = 0; var2 < player.getInventory().getContainerSize(); ++var2) {
            if (checkEnchantedPlaceholder(stack, player.getInventory().getItem(var2)) || areItemStacksEqual(player.getInventory().getItem(var2), stack, new ArcanaInvHelper.InvFilter(false, !stack.hasTag(), ore, false).setRelaxedNBT())) {
                count -= player.getInventory().getItem(var2).getCount();
                if (count <= 0) {
                    return true;
                }
    ...
    public static boolean checkEnchantedPlaceholder(ItemStack stack, ItemStack stack2) {
        return false;
//        if (stack.getItem() != ModItems.enchantedPlaceholder.get()) {
//            return false;
//        }
...
//        return b;
    ...
    public static boolean areItemStacksEqual(ItemStack stack0, ItemStack stack1, ArcanaInvHelper.InvFilter filter) {
        ...
        boolean t1 = true;
        if (!filter.igNBT) {
            t1 = (filter.relaxedNBT ? ArcanaInvHelper.areItemStackTagsEqualRelaxed(stack0, stack1) : ItemStack.isSameItemSameTags(stack0, stack1));
        }
        ...
        if (input instanceof Ingredient) {
            ...
            for (ItemStack s : ((Ingredient) input).getItems()) {
                if (s.isEmpty())
                    continue;
                if (!s.hasTag()) {
                    // TODO: revisit after creative tab
//                    s.getItem().fillItemCategory(CreativeModeTabs.searchTab(), lst);
                } else {
                    lst.add(s);
                }
            }
