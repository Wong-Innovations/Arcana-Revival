...
public class CardAnalyze extends TheorycraftCard {
    ...
    public boolean initialize(Player player, ResearchTableData data) {
        ...
        for (ResearchCategory rc : ResearchCategories.researchCategories.values()) {
            ...
            if (ModCapabilities.getKnowledge(player).getKnowledge(
    ...
    public boolean activate(Player player, ResearchTableData data) {
        ...
        int k = ModCapabilities.getKnowledge(player).getKnowledge(IPlayerKnowledge.EnumKnowledgeType.OBSERVATION, rc);
        if (k >= 1) {
            data.addTotal("BASICS", 5);
            ModCapabilities.getKnowledge(player).addKnowledge(
